...
function Signup({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createOrUpdateUserDoc(user, { displayName: name });
      
      const userData = { id: user.uid, email: user.email, name: name || user.email.split('@')[0], photoURL: user.photoURL };
      onLogin(userData);
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#FFF1B0] flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">Start your habit journey!</p>
        </div>
